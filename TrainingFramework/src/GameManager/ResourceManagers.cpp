...
void ResourceManagers::PlaySoundWithDuration(const std::string& name, float time)
{
	std::shared_ptr<SoLoud::Wav> wave;
	auto it = m_MapWave.find(name);
	if (it != m_MapWave.end())
	...
	SoLoud::handle a = m_Soloud.play(*wave, -1.0f, 0.0f, true);
	while (m_Soloud.getStreamTime(a) < time)
	{
		m_Soloud.setPause(a, false);
	}
	m_Soloud.stopAudioSource(*wave);
}

void ResourceManagers::PlaySoundAfterDelay(const std::string& name, float timeDelay, float timeDuration)
{
	std::shared_ptr<SoLoud::Wav> wave;
	auto it = m_MapWave.find(name);
	if (it != m_MapWave.end())
	...
	wave->setLooping(false);
	/*SoLoud::handle a = m_Soloud.play(*wave, -1.0f, 0.0f, true);*/
	SoLoud::handle a = m_Soloud.playClocked(timeDelay, *wave);
	/*while (m_Soloud.getStreamTime(a) < timeDuration)
	{
		m_Soloud.setPause(a, false);
	}
	m_Soloud.stopAudioSource(*wave);*/
